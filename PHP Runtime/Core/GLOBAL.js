import {define} from './Core';

export default class GLOBAL {
    static 'E_ERROR' = 1;

    /**
     * Catchable fatal error. It indicates that a probably dangerous error
     * occured= but did not leave the Engine in an unstable state. If the error
     * is not caught by a user defined handle (see also
     * <b>set_error_handler</b>)= the application aborts as it
     * was an <b>E_ERROR</b>.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_RECOVERABLE_ERROR' = 4096;

    /**
     * Run-time warnings (non-fatal errors). Execution of the script is not
     * halted.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_WARNING' = 2;

    /**
     * Compile-time parse errors. Parse errors should only be generated by
     * the parser.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_PARSE' = 4;

    /**
     * Run-time notices. Indicate that the script encountered something that
     * could indicate an error= but could also happen in the normal course of
     * running a script.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_NOTICE' = 8;

    /**
     * Enable to have PHP suggest changes
     * to your code which will ensure the best interoperability
     * and forward compatibility of your code.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_STRICT' = 2048;

    /**
     * Run-time notices. Enable this to receive warnings about code
     * that will not work in future versions.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_DEPRECATED' = 8192;

    /**
     * Fatal errors that occur during PHP's initial startup. This is like an
     * <b>E_ERROR</b>= except it is generated by the core of PHP.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_CORE_ERROR' = 16;

    /**
     * Warnings (non-fatal errors) that occur during PHP's initial startup.
     * This is like an <b>E_WARNING</b>= except it is generated
     * by the core of PHP.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_CORE_WARNING' = 32;

    /**
     * Fatal compile-time errors. This is like an <b>E_ERROR</b>,
     * except it is generated by the Zend Scripting Engine.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_COMPILE_ERROR' = 64;

    /**
     * Compile-time warnings (non-fatal errors). This is like an
     * <b>E_WARNING</b>= except it is generated by the Zend
     * Scripting Engine.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_COMPILE_WARNING' = 128;

    /**
     * User-generated error message. This is like an
     * <b>E_ERROR</b>= except it is generated in PHP code by
     * using the PHP function <b>trigger_error</b>.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_USER_ERROR' = 256;

    /**
     * User-generated warning message. This is like an
     * <b>E_WARNING</b>= except it is generated in PHP code by
     * using the PHP function <b>trigger_error</b>.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_USER_WARNING' = 512;

    /**
     * User-generated notice message. This is like an
     * <b>E_NOTICE</b>= except it is generated in PHP code by
     * using the PHP function <b>trigger_error</b>.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_USER_NOTICE' = 1024;

    /**
     * User-generated warning message. This is like an
     * <b>E_DEPRECATED</b>= except it is generated in PHP code by
     * using the PHP function <b>trigger_error</b>.
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_USER_DEPRECATED' = 16384;

    /**
     * All errors and warnings= as supported= except of level
     * <b>E_STRICT</b> prior to PHP 5.4.0.
     * Value of <b>E_ALL</b> is 32767 since PHP 5.4.x,
     * 30719 in PHP 5.3.x= 6143 in PHP 5.2.x= 2047 previously
     * @link https://php.net/manual/en/errorfunc.constants.php
     */
    static 'E_ALL' = 32767;
    static 'DEBUG_BACKTRACE_PROVIDE_OBJECT' = 1;
    static 'DEBUG_BACKTRACE_IGNORE_ARGS' = 2;
    static 'S_MEMORY' = 1;
    static 'S_VARS' = 4;
    static 'S_FILES' = 8;
    static 'S_INCLUDE' = 16;
    static 'S_SQL' = 32;
    static 'S_EXECUTOR' = 64;
    static 'S_MAIL' = 128;
    static 'S_SESSION' = 256;
    static 'S_MISC' = 2;
    static 'S_INTERNAL' = 536870912;
    static 'S_ALL' = 511;

    /**
     * system is unusable
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_EMERG' = 0;

    /**
     * action must be taken immediately
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_ALERT' = 1;

    /**
     * critical conditions
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_CRIT' = 2;

    /**
     * error conditions
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_ERR' = 3;

    /**
     * warning conditions
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_WARNING' = 4;

    /**
     * normal= but significant= condition
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_NOTICE' = 5;

    /**
     * informational message
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_INFO' = 6;

    /**
     * debug-level message
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_DEBUG' = 7;

    /**
     * kernel messages
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_KERN' = 0;

    /**
     * generic user-level messages
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_USER' = 8;

    /**
     * mail subsystem
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_MAIL' = 16;

    /**
     * other system daemons
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_DAEMON' = 24;

    /**
     * security/authorization messages (use <b>LOG_AUTHPRIV</b> instead
     * in systems where that constant is defined)
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_AUTH' = 32;

    /**
     * messages generated internally by syslogd
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_SYSLOG' = 40;

    /**
     * line printer subsystem
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_LPR' = 48;

    /**
     * USENET news subsystem
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_NEWS' = 56;

    /**
     * UUCP subsystem
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_UUCP' = 64;

    /**
     * clock daemon (cron and at)
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_CRON' = 72;

    /**
     * security/authorization messages (private)
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_AUTHPRIV' = 80;
    static 'LOG_LOCAL0' = 128;
    static 'LOG_LOCAL1' = 136;
    static 'LOG_LOCAL2' = 144;
    static 'LOG_LOCAL3' = 152;
    static 'LOG_LOCAL4' = 160;
    static 'LOG_LOCAL5' = 168;
    static 'LOG_LOCAL6' = 176;
    static 'LOG_LOCAL7' = 184;

    /**
     * include PID with each message
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_PID' = 1;

    /**
     * if there is an error while sending data to the system logger,
     * write directly to the system console
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_CONS' = 2;

    /**
     * (default) delay opening the connection until the first
     * message is logged
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_ODELAY' = 4;

    /**
     * open the connection to the logger immediately
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_NDELAY' = 8;
    static 'LOG_NOWAIT' = 16;

    /**
     * print log message also to standard error
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'LOG_PERROR' = 32;
    static 'TRUE' = 1;
    static 'FALSE' = 0;
    static 'NULL' = null;
    static 'ZEND_THREAD_SAFE' = false;
    static 'ZEND_DEBUG_BUILD' = false;
    static 'PHP_WINDOWS_VERSION_BUILD' = 0;
    static 'PHP_WINDOWS_VERSION_MAJOR' = 0;
    static 'PHP_WINDOWS_VERSION_MINOR' = 0;
    static 'PHP_WINDOWS_VERSION_PLATFORM' = 0;
    static 'PHP_WINDOWS_VERSION_PRODUCTTYPE' = 0;
    static 'PHP_WINDOWS_VERSION_SP_MAJOR' = 0;
    static 'PHP_WINDOWS_VERSION_SP_MINOR' = 0;
    static 'PHP_WINDOWS_VERSION_SUITEMASK' = 0;
    static 'PHP_VERSION' = '5.3.6-13ubuntu3.2';
    static 'PHP_MAJOR_VERSION' = 5;
    static 'PHP_MINOR_VERSION' = 3;
    static 'PHP_RELEASE_VERSION' = 6;
    static 'PHP_EXTRA_VERSION' = '-13ubuntu3.2';
    static 'PHP_VERSION_ID' = 50306;
    static 'PHP_ZTS' = 0;
    static 'PHP_DEBUG' = 0;
    static 'PHP_OS' = 'Linux';
    /**
     * The operating system family PHP was built for. Either of 'Windows'= 'BSD'= 'Darwin'= 'Solaris'= 'Linux' or
     * 'Unknown'. Available as of PHP 7.2.0.
     * @since 7.2
     */
    static 'PHP_OS_FAMILY' = 'Linux';
    static 'PHP_SAPI' = 'cli';
    static 'DEFAULT_INCLUDE_PATH' = '.:/usr/share/php:/usr/share/pear';
    static 'PEAR_INSTALL_DIR' = '/usr/share/php';
    static 'PEAR_EXTENSION_DIR' = '/usr/lib/php5/20090626';
    static 'PHP_EXTENSION_DIR' = '/usr/lib/php5/20090626';
    static 'PHP_PREFIX' = '/usr';
    static 'PHP_BINDIR' = '/usr/bin';
    static 'PHP_LIBDIR' = '/usr/lib/php5';
    static 'PHP_DATADIR' = '/usr/share';
    static 'PHP_SYSCONFDIR' = '/etc';
    static 'PHP_LOCALSTATEDIR' = '/var';
    static 'PHP_CONFIG_FILE_PATH' = '/etc/php5/cli';
    static 'PHP_CONFIG_FILE_SCAN_DIR' = '/etc/php5/cli/conf.d';
    static 'PHP_SHLIB_SUFFIX' = 'so';
    static 'PHP_EOL' = '\n';
    static 'SUHOSIN_PATCH' = 1;
    static 'SUHOSIN_PATCH_VERSION' = '0.9.10';
    static 'PHP_MAXPATHLEN' = 4096;
    static 'PHP_INT_MAX' = 9223372036854775807;
    static 'PHP_INT_MIN' = -9223372036854775808;
    static 'PHP_INT_SIZE' = 8;
    /**
     * Number of decimal digits that can be rounded into a float and back without precision loss. Available as of PHP 7.2.0.
     * @since 7.2
     */
    static 'PHP_FLOAT_DIG' = 15;
    /**
     * Smallest representable positive number x= so that x + 1.0 != 1.0. Available as of PHP 7.2.0.
     * @since 7.2
     */
    static 'PHP_FLOAT_EPSILON' = 2.2204460492503e-16;

    /**
     * Largest representable floating point number. Available as of PHP 7.2.0.
     * @since 7.2
     */
    static 'PHP_FLOAT_MAX' = 1.7976931348623e+308;
    /**
     * Smallest representable floating point number. Available as of PHP 7.2.0.
     * @since 7.2
     */
    static 'PHP_FLOAT_MIN' = 2.2250738585072e-308;
    static 'ZEND_MULTIBYTE' = 0;
    static 'PHP_OUTPUT_HANDLER_START' = 1;
    static 'PHP_OUTPUT_HANDLER_CONT' = 2;
    static 'PHP_OUTPUT_HANDLER_END' = 4;
    static 'UPLOAD_ERR_OK' = 0;
    static 'UPLOAD_ERR_INI_SIZE' = 1;
    static 'UPLOAD_ERR_FORM_SIZE' = 2;
    static 'UPLOAD_ERR_PARTIAL' = 3;
    static 'UPLOAD_ERR_NO_FILE' = 4;
    static 'UPLOAD_ERR_NO_TMP_DIR' = 6;
    static 'UPLOAD_ERR_CANT_WRITE' = 7;
    static 'UPLOAD_ERR_EXTENSION' = 8;

    static 'PHP_FD_SETSIZE' = 1024;
    static 'CONNECTION_ABORTED' = 1;
    static 'CONNECTION_NORMAL' = 0;
    static 'CONNECTION_TIMEOUT' = 2;
    static 'INI_USER' = 1;
    static 'INI_PERDIR' = 2;
    static 'INI_SYSTEM' = 4;
    static 'INI_ALL' = 7;

    /**
     * Normal INI scanner mode
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'INI_SCANNER_NORMAL' = 0;

    /**
     * Typed INI scanner mode
     * @since 5.6.1
     * @link https://php.net/manual/en/function.parse-ini-file.php
     */
    static 'INI_SCANNER_TYPED' = 2;

    /**
     * Raw INI scanner mode
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'INI_SCANNER_RAW' = 1;

    static 'PHP_URL_SCHEME' = 0;
    static 'PHP_URL_HOST' = 1;
    static 'PHP_URL_PORT' = 2;
    static 'PHP_URL_USER' = 3;
    static 'PHP_URL_PASS' = 4;
    static 'PHP_URL_PATH' = 5;
    static 'PHP_URL_QUERY' = 6;
    static 'PHP_URL_FRAGMENT' = 7;

    /**
     * <i>e</i> constant
     */
    static 'M_E' = 2.718281828459;

    /**
     * {@link log}<sub>2</sub><i>e</i> constant
     */
    static 'M_LOG2E' = 1.442695040889;

    /**
     * {@link log}<sub>10</sub><i>e</i> constant
     */
    static 'M_LOG10E' = 0.43429448190325;

    /**
     * {@link log}<sub><i>e</i></sub>2 constant
     */
    static 'M_LN2' = 0.69314718055995;

    /**
     * {@link log}<sub><i>e</i></sub>10 constant
     */
    static 'M_LN10' = 2.302585092994;

    /**
     * &pi; constant
     */
    static 'M_PI' = 3.1415926535898;

    /**
     * &pi;/2 constant
     */
    static 'M_PI_2' = 1.5707963267949;

    /**
     * &pi;/4 constant
     */
    static 'M_PI_4' = 0.78539816339745;

    /**
     * 1/&pi; constant
     */
    static 'M_1_PI' = 0.31830988618379;

    /**
     * 2/&pi; constant
     */
    static 'M_2_PI' = 0.63661977236758;

    /**
     * {@link sqrt}(&pi;) constant
     */
    static 'M_SQRTPI' = 1.7724538509055;

    /**
     * 2/{@link sqrt}(&pi;) constant
     */
    static 'M_2_SQRTPI' = 1.1283791670955;

    /**
     * {@link log}<sub><i>e</i></sub>&pi; constant
     */
    static 'M_LNPI' = 1.1447298858494;

    /**
     * Euler constant
     */
    static 'M_EULER' = 0.57721566490153;

    /**
     * {@link sqrt}(2) constant
     */
    static 'M_SQRT2' = 1.4142135623731;

    /**
     * 1/{@link sqrt}(2) constant
     */
    static 'M_SQRT1_2' = 0.70710678118655;

    /**
     * {@link sqrt}(3) constant
     */
    static 'M_SQRT3' = 1.7320508075689;

    /**
     * The infinite
     */
    static 'INF' = Number.MAX_VALUE;

    /**
     * Not A Number
     */
    static 'NAN' = NaN;

    /**
     * Round halves up
     * @link https://php.net/manual/en/math.constants.php
     */
    static 'PHP_ROUND_HALF_UP' = 1;

    /**
     * Round halves down
     * @link https://php.net/manual/en/math.constants.php
     */
    static 'PHP_ROUND_HALF_DOWN' = 2;

    /**
     * Round halves to even numbers
     * @link https://php.net/manual/en/math.constants.php
     */
    static 'PHP_ROUND_HALF_EVEN' = 3;

    /**
     * Round halves to odd numbers
     * @link https://php.net/manual/en/math.constants.php
     */
    static 'PHP_ROUND_HALF_ODD' = 4;
    static 'INFO_GENERAL' = 1;

    /**
     * PHP Credits. See also phpcredits.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_CREDITS' = 2;

    /**
     * Current Local and Master values for PHP directives. See
     * also ini_get.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_CONFIGURATION' = 4;

    /**
     * Loaded modules and their respective settings.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_MODULES' = 8;

    /**
     * Environment Variable information that's also available in
     * $_ENV.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_ENVIRONMENT' = 16;

    /**
     * Shows all
     * predefined variables from EGPCS (Environment=GET,
     * POST=Cookie=Server).
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_VARIABLES' = 32;

    /**
     * PHP License information. See also the license faq.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'INFO_LICENSE' = 64;
    static 'INFO_ALL' = 4294967295;

    /**
     * A list of the core developers
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_GROUP' = 1;

    /**
     * General credits: Language design and concept=PHP
     * authors and SAPI module.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_GENERAL' = 2;

    /**
     * A list of the server API modules for PHP=and their authors.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_SAPI' = 4;

    /**
     * A list of the extension modules for PHP=and their authors.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_MODULES' = 8;

    /**
     * The credits for the documentation team.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_DOCS' = 16;

    /**
     * Usually used in combination with the other flags. Indicates
     * that a complete stand-alone HTML page needs to be
     * printed including the information indicated by the other
     * flags.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_FULLPAGE' = 32;

    /**
     * The credits for the quality assurance team.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_QA' = 64;

    /**
     * The configuration line=&php.ini; location=build date=Web
     * Server=System and more.
     * @link https://php.net/manual/en/info.constants.php
     */
    static 'CREDITS_ALL' = 4294967295;
    static 'HTML_SPECIALCHARS' = 0;
    static 'HTML_ENTITIES' = 1;

    /**
     * Will convert double-quotes and leave single-quotes alone.
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_COMPAT' = 2;

    /**
     * Will convert both double and single quotes.
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_QUOTES' = 3;

    /**
     * Will leave both double and single quotes unconverted.
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_NOQUOTES' = 0;

    /**
     * Silently discard invalid code unit sequences instead of returning an empty string.
     * Using this flag is discouraged as it may have security implications.
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_IGNORE' = 4;
    static 'STR_PAD_LEFT' = 0;
    static 'STR_PAD_RIGHT' = 1;
    static 'STR_PAD_BOTH' = 2;
    static 'PATHINFO_DIRNAME' = 1;
    static 'PATHINFO_BASENAME' = 2;
    static 'PATHINFO_EXTENSION' = 4;

    /**
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'PATHINFO_FILENAME' = 8;
    static 'CHAR_MAX' = 127;
    static 'LC_CTYPE' = 0;
    static 'LC_NUMERIC' = 1;
    static 'LC_TIME' = 2;
    static 'LC_COLLATE' = 3;
    static 'LC_MONETARY' = 4;
    static 'LC_ALL' = 6;
    static 'LC_MESSAGES' = 5;
    static 'SEEK_SET' = 0;
    static 'SEEK_CUR' = 1;
    static 'SEEK_END' = 2;

    /**
     * Acquire a shared lock (reader).
     * @link https://www.php.net/manual/en/function.flock.php
     */
    static 'LOCK_SH' = 1;

    /**
     * Acquire an exclusive lock (writer).
     * @link https://www.php.net/manual/en/function.flock.php
     */
    static 'LOCK_EX' = 2;

    /**
     * Release lock (shared or exclusive).
     * @link https://www.php.net/manual/en/function.flock.php
     */
    static 'LOCK_UN' = 3;

    /**
     * Non-blocking operation while locking.
     * @link https://www.php.net/manual/en/function.flock.php
     */
    static 'LOCK_NB' = 4;

    /**
     * A connection with an external resource has been established.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_CONNECT' = 2;

    /**
     * Additional authorization is required to access the specified resource.
     * Typical issued with severity level of
     * STREAM_NOTIFY_SEVERITY_ERR.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_AUTH_REQUIRED' = 3;

    /**
     * Authorization has been completed (with or without success).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_AUTH_RESULT' = 10;

    /**
     * The mime-type of resource has been identified,
     * refer to message for a description of the
     * discovered type.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_MIME_TYPE_IS' = 4;

    /**
     * The size of the resource has been discovered.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_FILE_SIZE_IS' = 5;

    /**
     * The external resource has redirected the stream to an alternate
     * location. Refer to message.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_REDIRECTED' = 6;

    /**
     * Indicates current progress of the stream transfer in
     * bytes_transferred and possibly
     * bytes_max as well.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_PROGRESS' = 7;

    /**
     * A generic error occurred on the stream=consult
     * message and message_code
     * for details.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_FAILURE' = 9;

    /**
     * There is no more data available on the stream.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_COMPLETED' = 8;

    /**
     * A remote address required for this stream has been resolved=or the resolution
     * failed. See severity for an indication of which happened.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_RESOLVE' = 1;

    /**
     * Normal=non-error related=notification.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_SEVERITY_INFO' = 0;

    /**
     * Non critical error condition. Processing may continue.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_SEVERITY_WARN' = 1;

    /**
     * A critical error occurred. Processing cannot continue.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_NOTIFY_SEVERITY_ERR' = 2;

    /**
     * Used with stream_filter_append and
     * stream_filter_prepend to indicate
     * that the specified filter should only be applied when
     * reading
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_FILTER_READ' = 1;

    /**
     * Used with stream_filter_append and
     * stream_filter_prepend to indicate
     * that the specified filter should only be applied when
     * writing
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_FILTER_WRITE' = 2;

    /**
     * This constant is equivalent to
     * STREAM_FILTER_READ | STREAM_FILTER_WRITE
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_FILTER_ALL' = 3;

    /**
     * Client socket opened with stream_socket_client
     * should remain persistent between page loads.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_CLIENT_PERSISTENT' = 1;

    /**
     * Open client socket asynchronously. This option must be used
     * together with the STREAM_CLIENT_CONNECT flag.
     * Used with stream_socket_client.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_CLIENT_ASYNC_CONNECT' = 2;

    /**
     * Open client socket connection. Client sockets should always
     * include this flag. Used with stream_socket_client.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_CLIENT_CONNECT' = 4;

    /**
     * Used with stream_socket_shutdown to disable
     * further receptions.
     * @since 5.2.1
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SHUT_RD' = 0;

    /**
     * Used with stream_socket_shutdown to disable
     * further transmissions.
     * @since 5.2.1
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SHUT_WR' = 1;

    /**
     * Used with stream_socket_shutdown to disable
     * further receptions and transmissions.
     * @since 5.2.1
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SHUT_RDWR' = 2;

    /**
     * Internet Protocol Version 4 (IPv4).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_PF_INET' = 2;

    /**
     * Internet Protocol Version 6 (IPv6).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_PF_INET6' = 10;

    /**
     * Unix system internal protocols.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_PF_UNIX' = 1;

    /**
     * Provides a IP socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_IPPROTO_IP' = 0;

    /**
     * Provides a TCP socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_IPPROTO_TCP' = 6;

    /**
     * Provides a UDP socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_IPPROTO_UDP' = 17;

    /**
     * Provides a ICMP socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_IPPROTO_ICMP' = 1;

    /**
     * Provides a RAW socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_IPPROTO_RAW' = 255;

    /**
     * Provides sequenced=two-way byte streams with a transmission mechanism
     * for out-of-band data (TCP=for example).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SOCK_STREAM' = 1;

    /**
     * Provides datagrams=which are connectionless messages (UDP=for
     * example).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SOCK_DGRAM' = 2;

    /**
     * Provides a raw socket=which provides access to internal network
     * protocols and interfaces. Usually this type of socket is just available
     * to the root user.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SOCK_RAW' = 3;

    /**
     * Provides a sequenced packet stream socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SOCK_SEQPACKET' = 5;

    /**
     * Provides a RDM (Reliably-delivered messages) socket.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SOCK_RDM' = 4;
    static 'STREAM_PEEK' = 2;
    static 'STREAM_OOB' = 1;

    /**
     * Tells a stream created with stream_socket_server
     * to bind to the specified target. Server sockets should always include this flag.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SERVER_BIND' = 4;

    /**
     * Tells a stream created with stream_socket_server
     * and bound using the STREAM_SERVER_BIND flag to start
     * listening on the socket. Connection-orientated transports (such as TCP)
     * must use this flag=otherwise the server socket will not be enabled.
     * Using this flag for connect-less transports (such as UDP) is an error.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_SERVER_LISTEN' = 8;

    /**
     * Search for filename in include_path
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_USE_INCLUDE_PATH' = 1;

    /**
     * Strip EOL characters
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_IGNORE_NEW_LINES' = 2;

    /**
     * Skip empty lines
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_SKIP_EMPTY_LINES' = 4;

    /**
     * Append content to existing file.
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_APPEND' = 8;
    static 'FILE_NO_DEFAULT_CONTEXT' = 16;

    /**
     * <p>
     * This constant has no effect prior to PHP 6. It is only available for
     * forward compatibility.
     * </p>
     * @since 5.2.7
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_TEXT' = 0;

    /**
     * <p>
     * This constant has no effect prior to PHP 6. It is only available for
     * forward compatibility.
     * </p>
     * @since 5.2.7
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FILE_BINARY' = 0;

    /**
     * Disable backslash escaping.
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FNM_NOESCAPE' = 2;

    /**
     * Slash in string only matches slash in the given pattern.
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FNM_PATHNAME' = 1;

    /**
     * Leading period in string must be exactly matched by period in the given pattern.
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FNM_PERIOD' = 4;

    /**
     * Caseless match. Part of the GNU extension.
     * @link https://php.net/manual/en/filesystem.constants.php
     */
    static 'FNM_CASEFOLD' = 16;

    /**
     * Return Code indicating that the
     * userspace filter returned buckets in $out.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_PASS_ON' = 2;

    /**
     * Return Code indicating that the
     * userspace filter did not return buckets in $out
     * (i.e. No data available).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_FEED_ME' = 1;

    /**
     * Return Code indicating that the
     * userspace filter encountered an unrecoverable error
     * (i.e. Invalid data received).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_ERR_FATAL' = 0;

    /**
     * Regular read/write.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_FLAG_NORMAL' = 0;

    /**
     * An incremental flush.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_FLAG_FLUSH_INC' = 1;

    /**
     * Final flush prior to closing.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'PSFS_FLAG_FLUSH_CLOSE' = 2;
    static 'ABDAY_1' = 131072;
    static 'ABDAY_2' = 131073;
    static 'ABDAY_3' = 131074;
    static 'ABDAY_4' = 131075;
    static 'ABDAY_5' = 131076;
    static 'ABDAY_6' = 131077;
    static 'ABDAY_7' = 131078;
    static 'DAY_1' = 131079;
    static 'DAY_2' = 131080;
    static 'DAY_3' = 131081;
    static 'DAY_4' = 131082;
    static 'DAY_5' = 131083;
    static 'DAY_6' = 131084;
    static 'DAY_7' = 131085;
    static 'ABMON_1' = 131086;
    static 'ABMON_2' = 131087;
    static 'ABMON_3' = 131088;
    static 'ABMON_4' = 131089;
    static 'ABMON_5' = 131090;
    static 'ABMON_6' = 131091;
    static 'ABMON_7' = 131092;
    static 'ABMON_8' = 131093;
    static 'ABMON_9' = 131094;
    static 'ABMON_10' = 131095;
    static 'ABMON_11' = 131096;
    static 'ABMON_12' = 131097;
    static 'MON_1' = 131098;
    static 'MON_2' = 131099;
    static 'MON_3' = 131100;
    static 'MON_4' = 131101;
    static 'MON_5' = 131102;
    static 'MON_6' = 131103;
    static 'MON_7' = 131104;
    static 'MON_8' = 131105;
    static 'MON_9' = 131106;
    static 'MON_10' = 131107;
    static 'MON_11' = 131108;
    static 'MON_12' = 131109;
    static 'AM_STR' = 131110;
    static 'PM_STR' = 131111;
    static 'D_T_FMT' = 131112;
    static 'D_FMT' = 131113;
    static 'T_FMT' = 131114;
    static 'T_FMT_AMPM' = 131115;
    static 'ERA' = 131116;
    static 'ERA_D_T_FMT' = 131120;
    static 'ERA_D_FMT' = 131118;
    static 'ERA_T_FMT' = 131121;
    static 'ALT_DIGITS' = 131119;
    static 'CRNCYSTR' = 262159;
    static 'RADIXCHAR' = 65536;
    static 'THOUSEP' = 65537;
    static 'YESEXPR' = 327680;
    static 'NOEXPR' = 327681;
    static 'YESSTR' = 54;
    static 'NOSTR' = 55;
    static 'CODESET' = 14;
    static 'CRYPT_SALT_LENGTH' = 123;
    static 'CRYPT_STD_DES' = 1;
    static 'CRYPT_EXT_DES' = 1;
    static 'CRYPT_MD5' = 1;
    static 'CRYPT_BLOWFISH' = 1;
    static 'CRYPT_SHA256' = 1;
    static 'CRYPT_SHA512' = 1;
    static 'DIRECTORY_SEPARATOR' = '/';
    static 'PATH_SEPARATOR' = ':';
    static 'GLOB_BRACE' = 1024;
    static 'GLOB_MARK' = 2;
    static 'GLOB_NOSORT' = 4;
    static 'GLOB_NOCHECK' = 16;
    static 'GLOB_NOESCAPE' = 64;
    static 'GLOB_ERR' = 1;
    static 'GLOB_ONLYDIR' = 8192;
    static 'GLOB_AVAILABLE_FLAGS' = 9303;
    static 'EXTR_OVERWRITE' = 0;
    static 'EXTR_SKIP' = 1;
    static 'EXTR_PREFIX_SAME' = 2;
    static 'EXTR_PREFIX_ALL' = 3;
    static 'EXTR_PREFIX_INVALID' = 4;
    static 'EXTR_PREFIX_IF_EXISTS' = 5;
    static 'EXTR_IF_EXISTS' = 6;
    static 'EXTR_REFS' = 256;

    /**
     * SORT_ASC is used with
     * array_multisort to sort in ascending order.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_ASC' = 4;

    /**
     * SORT_DESC is used with
     * array_multisort to sort in descending order.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_DESC' = 3;

    /**
     * SORT_REGULAR is used to compare items normally.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_REGULAR' = 0;

    /**
     * SORT_NUMERIC is used to compare items numerically.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_NUMERIC' = 1;

    /**
     * SORT_STRING is used to compare items as strings.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_STRING' = 2;

    /**
     * SORT_LOCALE_STRING is used to compare items as
     * strings=based on the current locale.
     * @since 5.0.2
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_LOCALE_STRING' = 5;

    /**
     * CASE_LOWER is used with
     * array_change_key_case and is used to convert array
     * keys to lower case. This is also the default case for
     * array_change_key_case.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'CASE_LOWER' = 0;

    /**
     * CASE_UPPER is used with
     * array_change_key_case and is used to convert array
     * keys to upper case.
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'CASE_UPPER' = 1;
    static 'COUNT_NORMAL' = 0;
    static 'COUNT_RECURSIVE' = 1;
    static 'ASSERT_ACTIVE' = 1;
    static 'ASSERT_CALLBACK' = 2;
    static 'ASSERT_BAIL' = 3;
    static 'ASSERT_WARNING' = 4;
    static 'ASSERT_QUIET_EVAL' = 5;
    static 'ASSERT_EXCEPTION' = 6;

    /**
     * Flag indicating if the stream used the include path.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_USE_PATH' = 1;
    static 'STREAM_IGNORE_URL' = 2;
    static 'STREAM_ENFORCE_SAFE_MODE' = 4;

    /**
     * Flag indicating if the wrapper
     * is responsible for raising errors using trigger_error
     * during opening of the stream. If this flag is not set=you
     * should not raise any errors.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_REPORT_ERRORS' = 8;

    /**
     * This flag is useful when your extension really must be able to randomly
     * seek around in a stream. Some streams may not be seekable in their
     * native form=so this flag asks the streams API to check to see if the
     * stream does support seeking. If it does not=it will copy the stream
     * into temporary storage (which may be a temporary file or a memory
     * stream) which does support seeking.
     * Please note that this flag is not useful when you want to seek the
     * stream and write to it=because the stream you are accessing might
     * not be bound to the actual resource you requested.
     * If the requested resource is network based=this flag will cause the
     * opener to block until the whole contents have been downloaded.
     * @link https://php.net/manual/en/internals2.ze1.streams.constants.php
     */
    static 'STREAM_MUST_SEEK' = 16;
    static 'STREAM_URL_STAT_LINK' = 1;
    static 'STREAM_URL_STAT_QUIET' = 2;
    static 'STREAM_MKDIR_RECURSIVE' = 1;
    static 'STREAM_IS_URL' = 1;
    static 'STREAM_OPTION_BLOCKING' = 1;
    static 'STREAM_OPTION_READ_TIMEOUT' = 4;
    static 'STREAM_OPTION_READ_BUFFER' = 2;
    static 'STREAM_OPTION_WRITE_BUFFER' = 3;
    static 'STREAM_BUFFER_NONE' = 0;
    static 'STREAM_BUFFER_LINE' = 1;
    static 'STREAM_BUFFER_FULL' = 2;

    /**
     * Stream casting=when stream_cast is called
     * otherwise (see above).
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_CAST_AS_STREAM' = 0;

    /**
     * Stream casting=for when stream_select is
     * calling stream_cast.
     * @link https://php.net/manual/en/stream.constants.php
     */
    static 'STREAM_CAST_FOR_SELECT' = 3;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_GIF' = 1;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JPEG' = 2;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_PNG' = 3;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_SWF' = 4;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_PSD' = 5;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_BMP' = 6;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_TIFF_II' = 7;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_TIFF_MM' = 8;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JPC' = 9;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JP2' = 10;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JPX' = 11;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JB2' = 12;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_SWC' = 13;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_IFF' = 14;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_WBMP' = 15;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_JPEG2000' = 9;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_XBM' = 16;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     */
    static 'IMAGETYPE_ICO' = 17;

    /**
     * Image type constant used by the {@link image_type_to_mime_type()} and {@link image_type_to_extension()} functions.
     * @link https://php.net/manual/en/image.constants.php
     * @since 7.1
     */
    static 'IMAGETYPE_WEBP' = 18;
    static 'IMAGETYPE_UNKNOWN' = 0;
    static 'IMAGETYPE_COUNT' = 19;

    /**
     * IPv4 Address Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_A' = 1;

    static 'DNS_CAA' = 8192;

    /**
     * Authoritative Name Server Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_NS' = 2;

    /**
     * Alias (Canonical Name) Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_CNAME' = 16;

    /**
     * Start of Authority Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_SOA' = 32;

    /**
     * Pointer Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_PTR' = 2048;

    /**
     * Host Info Resource (See IANA's
     * Operating System Names
     * for the meaning of these values)
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_HINFO' = 4096;

    /**
     * Mail Exchanger Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_MX' = 16384;

    /**
     * Text Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_TXT' = 32768;
    static 'DNS_SRV' = 33554432;
    static 'DNS_NAPTR' = 67108864;

    /**
     * IPv6 Address Resource
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_AAAA' = 134217728;
    static 'DNS_A6' = 16777216;

    /**
     * Any Resource Record. On most systems
     * this returns all resource records=however
     * it should not be counted upon for critical
     * uses. Try DNS_ALL instead.
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_ANY' = 268435456;

    /**
     * Iteratively query the name server for
     * each available record type.
     * @link https://php.net/manual/en/network.constants.php
     */
    static 'DNS_ALL' = 251721779;

// End of standard v.5.3.1-0.dotdeb.1


//WI-11084 Constant not defined PHP_QUERY_RFC3986

    /**
     * Encoding is performed per RFC 1738 and the application/x-www-form-urlencoded media type,
     * which implies that spaces are encoded as plus (+) signs.
     * @link https://php.net/manual/en/function.http-build-query.php
     */
    static 'PHP_QUERY_RFC1738' = 1;
    /**
     * Encoding is performed according to RFC 3986=and spaces will be percent encoded (%20).
     * @link https://php.net/manual/en/function.http-build-query.php
     */
    static 'PHP_QUERY_RFC3986' = 2;


//WI-11254 Stubs for missing constants from PHP 5.4

    /**
     * Specifies where the manpages were installed into.
     * @since PHP 5.3.7
     * @link https://php.net/manual/en/reserved.constants.php
     */
    static 'PHP_MANDIR' = '/usr/local/php/php/man';
    /**
     * Specifies where the binaries were installed into.
     * @link https://php.net/manual/en/reserved.constants.php
     */
    static 'PHP_BINARY' = '/usr/local/php/bin/php';

    /**
     * @since 7.4
     */
    static 'PHP_CLI_PROCESS_TITLE' = 1;


    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_WRITE' = 0;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_FLUSH' = 4;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_CLEAN' = 2;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_FINAL' = 8;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_CLEANABLE' = 16;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_FLUSHABLE' = 32;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_REMOVABLE' = 64;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_STDFLAGS' = 112;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_STARTED' = 4096;
    /** @link https://php.net/manual/en/outcontrol.constants.php */
    static 'PHP_OUTPUT_HANDLER_DISABLED' = 8192;


    /**
     * (PHP4=PHP5)
     * <p>Constant containing either the session name and session ID in the form of "name=ID" or
     * empty string if session ID was set in an appropriate session cookie.
     * This is the same id as the one returned by session_id().
     * @see session_id()
     * @link https://php.net/manual/en/session.constants.php
     */
    static 'SID' = 'name=ID';
    /**
     * Return value of session_status() if sessions are disabled.
     * @since 5.4
     * @link https://php.net/manual/en/function.session-status.php
     */
    static 'PHP_SESSION_DISABLED' = 0;
    /**
     * Return value of session_status() if sessions are enabled=but no session exists.
     * @since 5.4
     * @link https://php.net/manual/en/function.session-status.php
     */
    static 'PHP_SESSION_NONE' = 1;
    /**
     * Return value of session_status() if sessions are enabled=and a session exists.
     * @since 5.4
     * @link https://php.net/manual/en/function.session-status.php
     */
    static 'PHP_SESSION_ACTIVE' = 2;

    /**
     * Replace invalid code unit sequences with a Unicode Replacement Character
     * U+FFFD (UTF-8) or &#FFFD; (otherwise) instead of returning an empty string.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_SUBSTITUTE' = 8;
    /**
     * Replace invalid code points for the given document type with
     * a Unicode Replacement Character U+FFFD (UTF-8) or &#FFFD;
     * (otherwise) instead of leaving them as is. This may be useful,
     * for instance=to ensure the well-formedness of XML documents
     * with embedded external content.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_DISALLOWED' = 128;
    /**
     * Handle code as HTML 4.01.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_HTML401' = 0;
    /**
     * Handle code as XML 1.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_XML1' = 16;
    /**
     * Handle code as XHTML.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_XHTML' = 32;
    /**
     * Handle code as HTML 5.
     * @since 5.4
     * @link https://php.net/manual/en/function.htmlspecialchars.php
     */
    static 'ENT_HTML5' = 48;


    /** @link https://php.net/manual/en/function.scandir.php */
    static 'SCANDIR_SORT_ASCENDING' = 0;
    /** @link https://php.net/manual/en/function.scandir.php */
    static 'SCANDIR_SORT_DESCENDING' = 1;
    /** @link https://php.net/manual/en/function.scandir.php */
    static 'SCANDIR_SORT_NONE' = 2;


    /**
     * SORT_NATURAL is used to compare items as strings using "natural ordering" like natsort().
     * @since 5.4
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_NATURAL' = 6;
    /**
     * SORT_FLAG_CASE can be combined (bitwise OR) with SORT_STRING or SORT_NATURAL to sort strings case-insensitively.
     * @since 5.4
     * @link https://php.net/manual/en/array.constants.php
     */
    static 'SORT_FLAG_CASE' = 8;


    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_TOUCH' = 1;
    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_OWNER' = 3;
    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_OWNER_NAME' = 2;
    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_GROUP' = 5;
    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_GROUP_NAME' = 4;
    /** @link https://php.net/manual/en/streamwrapper.stream-metadata.php */
    static 'STREAM_META_ACCESS' = 6;


}
